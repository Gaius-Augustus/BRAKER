...
sub new_species {
    print LOG "\# " . (localtime) . ": Creating new_species parameter "
        . "directory for AUGUSTUS\n" if ($v > 2);
            ...
            )
        {
            ...
            if ( $gb_good_size == 0 ) {
                $prtStr
                    ...
                    . "be optimized. Recomended are at least 300 genes\n";
                print LOG $prtStr;
                clean_abort("$AUGUSTUS_CONFIG_PATH/species/$species",
                    $useexisting, $prtStr);
            }
            if ( $gb_good_size < 600 ) {
                ...
                if( $testsize1 == 0 or $testsize2 == 0 or ($gb_good_size - ($testsize1 + $testsize2)) == 0 ){
                    $prtStr = "\# "
                            ...
                            . "too low)! The provided input data is not "
...
sub eval_gene_pred {
    ...
    $cmdString = "$eval_multi_gtf $otherfilesDir/seqlist $annot $secondStepFile > $gtfFile.eval.out 2> $errorfilesDir/eval.$gtfFile.err";
    print LOG $cmdString."\n" if ($v > 3);
    system("$cmdString") == 0 or die("ERROR in file " . __FILE__ ." at line "
        . __LINE__ ."\nFailed to execute $cmdString\n");
    print LOG "\# "
...
sub train_utr {
    ...
    for ( ( "$species" . "_exon_probs.pbl", "$species" . "_igenic_probs.pbl",
            "$species" . "_intron_probs.pbl" ) )
    {
    ...
    if ( !uptodate( [ "utrs.gff",    "augustus.noUtr.gtf" ], [ "bothutr.lst", "bothutr.test.gb" ] ) )
    {
        ...
        open( UTR, "<", "utrs.gff" ) or die("ERROR in file " . __FILE__
        ...
        close(UTR)   or die("ERROR in file " . __FILE__ ." at line "
        ...
        system($cmdString) == 0 or die("ERROR in file " . __FILE__
            . " at line ". __LINE__ ."\nFailed not execute $cmdString!\n");
        open( TR, "tr.lst" ) or die("ERROR in file " . __FILE__
        ...
        close(TR)   or die("ERROR in file " . __FILE__ . " at line "
            . __LINE__ ."\nCould not close file tr.lst!\n");
        ...
        $perlCmdString .= "perl $string genes.gtf $genome $flanking_DNA bothutr.test.gb --good=bothutr.lst 1> $otherfilesDir/gff2gbSmallDNA.utr.stdout "
    ...
    if ( !uptodate( [ "bothutr.test.gb", "../training.gb.train.test" ], [ "train.gb",        "onlytrain.gb" ] ) )
    {
        ...
        close(NOMRNA) or die("ERROR in file " . __FILE__ ." at line "
        ...
        close(CDSONLY) or die("ERROR in file " . __FILE__ ." at line "
        ...
        close(TRAIN)  or die("ERROR in file " . __FILE__ . " at line "
            . __LINE__ ."\nCould not close file train.gb!\n");
        close(REMOVE) or die("ERROR in file " . __FILE__ ." at line "
