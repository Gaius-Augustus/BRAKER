...
startAlign.pl  split genome file in single sequences or sequence parts and protein file according 
               ...
               for each contigID or each sequence

SYNOPSIS

startAlign.pl [OPTIONS] --genome=genome.fa --prot=db.fa --list=BLAST.hit.list --pos=dna.pos --prg=gth|exonerate|spaln
...
OPTIONS
    ...
    --noreg                      Use whole sequences and not region parts.
...
my $noreg = 0;              # use whole sequences
...
# check whether position file is specified
if(defined($pos_file)){
  ...
  print STDERR "ERROR: No position file specified. Please check.\n";
  exit(1);
}


# check whether list file is specified
if(defined($list_file)){
  ...
  print STDERR "ERROR: No blast hit list file specified. Please check.\n";
  exit(1);
}
...
if(defined($prot_file)){
  # check whether list protein exists
  if(! -e $prot_file){
...
sub start_align{
  ...
  if(!$noreg){
    ...
    foreach my $ID (keys %seq){
      ...
      my $query = "prot$ID.fa";     # protein file
