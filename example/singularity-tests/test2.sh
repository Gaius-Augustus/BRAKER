...
singularity exec -B $PWD:$PWD braker3.sif braker.pl --genome=/opt/BRAKER/example/genome.fa --prot_seq=/opt/BRAKER/example/proteins.fa --softmasking --workingdir=${wd} --threads 8 --GENEMARK_PATH=${ETP}/bin/gmes --PROTHINT_PATH=${ETP}/bin/gmes/ProtHint/bin \
