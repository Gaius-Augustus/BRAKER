...
if [ ! -d $oldDir ] ; then
  echo "ERROR: Directory (with contents) of old BRAKER run $oldDir does not exist, yet. Please run test3.sh before running test3_restart3.sh!"
    ...
    ( time braker.pl --genome=../genome.fa --hints=$oldDir/hintsfile.gff --skipAllTraining --species=$species --etpmode --softmasking --workingdir=$wd --threads 8 ) &> test3_restart3.log
fi
