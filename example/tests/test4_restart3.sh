...
if [ ! -d $oldDir ] ; then
  echo "ERROR: Directory (with contents) of old BRAKER run $oldDir does not exist, yet. Please run test4.sh before running test4_restart3.sh!"
    ...
    ( time braker.pl --genome=../genome2.fa --hints=$oldDir/hintsfile.gff --skipAllTraining --species=$species --softmasking --workingdir=$wd --prg=gth --cores 8 ) &> test4_restart3.log
fi
