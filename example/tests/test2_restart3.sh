...
if [ ! -d $oldDir ] ; then
  echo "ERROR: Directory (with contents) of old BRAKER run $oldDir does not exist, yet. Please run test2.sh before running test2_restart2.sh!"
    ...
    ( time braker.pl --genome=../genome2.fa --hints=$oldDir/hintsfile.gff --skipAllTraining --species=$species --epmode --softmasking --workingdir=$wd --cores 8 ) &> test2_restart3.log
fi
