...
Table \ref{compare} shows gene prediction accuracy of GeneMark-ET (\textit{ab initio}, first step in BRAKER1), of AUGUSTUS \textit{ab initio} (trained on filtered GeneMark-ET predictions) and of BRAKER1 (AUGUSTUS using RNA-Seq read alignment information as extrinsic evidence).
...
\end{scriptsize}
\end{center}
\caption{Accuracy results of GeneMark-ET (\textit{ab initio}), AUGUSTUS (\textit{ab initio}) and BRAKER1 (AUGUSTUS trained on filtered GeneMark-ET predictions; run with RNA-Seq hints) on \textit{unmasked} genomes.\label{unmasked}}
\end{table}
...
%Transcript specificity  & 11.2 & 32.1 & 44.3 & \textbf{52.5} & 15.3 & 25.7 & \textbf{41.1} & 30.8\\
...
%Transcript specificity & 30.6 & 37.6 & \textbf{50.3} & 47.9 & 49.4 & 67.4 & \textbf{84.2} & 68.7\\
...
\begin{table}[h!]
\caption{\textit{Ab initio} prediction accuracy of AUGUSTUS. Mode 1: training on expert generated sets of genes; Mode 2: training on BRAKER1 generated set of genes. (Transcript specificity is not shown since it is in this case identical to Gene specificity; AUGUSTUS did not predict alternative transcripts.) \label{single_preds_braker}}
\begin{center}
