...
\item
  ...
  \item
    ...
    \tightlist
    \item
      \protect\hyperlink{braker1}{BRAKER2 with RNA-Seq data (only)}
    \item
      \protect\hyperlink{braker2-with-proteins-of-longer-evolutionary-distance}{BRAKER2
      with proteins of longer evolutionary distance}
    \item
      \protect\hyperlink{prot-in}{BRAKER2 with proteins of shorter
      evolutionary distance}
    \item
      \protect\hyperlink{braker2-with-rna-seq-and-protein-data}{BRAKER2
      with RNA-Seq and protein data}
    \end{itemize}
  \item
    \protect\hyperlink{options}{Description of selected BRAKER2 command
    line options}

    \begin{itemize}
    \tightlist
    \item
      \protect\hyperlink{ab_initio}{\texttt{–ab\_initio}}
    \item
      \protect\hyperlink{augustus_argssome_argbla}{\texttt{–augustus\_args=–some\_arg=bla}}
    \item
      \protect\hyperlink{coresint}{\texttt{–cores=INT}}
    \item
      \protect\hyperlink{fungus}{\texttt{–fungus}}
    \item
      \protect\hyperlink{softmasking}{\texttt{–softmasking}}
    \item
      \protect\hyperlink{useexisting}{\texttt{–useexisting}}
    \item
      \protect\hyperlink{crf}{\texttt{–crf}}
    \item
      \protect\hyperlink{lambdaint}{\texttt{–lambda=int}}
    \item
      \protect\hyperlink{utron}{\texttt{–UTR=on}}
    \item
      \protect\hyperlink{stranded-....}{\texttt{–stranded=+,-,.,...}}
    \end{itemize}
  \end{itemize}
\item
  \protect\hyperlink{output-of-braker2}{Output of BRAKER2}
\item
  \protect\hyperlink{example-data}{Example data}

  \begin{itemize}
  \tightlist
  \item
    \protect\hyperlink{data-description}{Data description}
  \item
    \protect\hyperlink{testing-braker2-with-rna-seq-only-data-test1.sh}{Testing
    BRAKER2 with RNA-Seq (only) data (\texttt{test1.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-hints-from-proteins-of-remote-homology-only-test2.sh}{Testing
    BRAKER2 with hints from proteins of remote homology (only)
    (\texttt{test2.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-hints-from-proteins-of-remote-homology-and-rna-seq-test3.sh}{Testing
    BRAKER2 with hints from proteins of remote homology and RNA-Seq
    (\texttt{test3.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-proteins-of-close-homology-only-test4.sh}{Testing
    BRAKER2 with proteins of close homology (only) (\texttt{test4.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-proteins-of-close-homology-and-rna-seq-data-rna-seq-supported-training-test5.sh}{Testing
    BRAKER2 with proteins of close homology and RNA-Seq data (RNA-Seq
    supported training) (\texttt{test5.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-proteins-of-close-homoogy-and-rna-seq-data-rna-seq-and-protein-supported-training-test6.sh}{Testing
    BRAKER2 with proteins of close homoogy and RNA-Seq data (RNA-Seq and
    protein supported training) (\texttt{test6.sh}}
  \item
    \protect\hyperlink{testing-braker2-with-pre-trained-parameters-prediction-only-test7.sh}{Testing
    BRAKER2 with pre-trained parameters (prediction only)
    (\texttt{test7.sh})}
  \item
    \protect\hyperlink{testing-braker2-with-genome-sequence-only-text8.sh}{Testing
    BRAKER2 with genome sequence, only (\texttt{text8.sh})}
  \end{itemize}
\item
  \protect\hyperlink{bug-reporting}{Bug reporting}

  \begin{itemize}
  \tightlist
  \item
    \protect\hyperlink{reporting-bugs-on-github}{Reporting bugs on
    github}
  \item
    \protect\hyperlink{commonproblems}{Common problems}
  \end{itemize}
\item
  \protect\hyperlink{citing-braker2-and-software-called-by-braker2}{Citing
  BRAKER2 and software called by BRAKER2}
\item
...
\item
  ...
  \emph{ab initio} gene prediction with AUGUSTUS.{[}braker-main-a{]}}
  \end{figure}
\item
  ...
  spliced alignment information.{[}braker-main-b{]}}
  \end{figure}
\item
  ...
  evolutionary distance.{[}braker-main-c{]}}
  \end{figure}
\item
  ...
  {[}braker-main-d{]}); \textbf{important:} this approach requires a
  ...
  GeneMark-ETP and AUGUSTUS is enforced.{[}braker-main-d{]}}
  \end{figure}
\item
  genome file and file with proteins of short evolutionary distance (see
  figure {[}braker2-sidetrack-b{]}); this approach is suitable if
  ...
  species against the target genome.{[}braker2-sidetrack-b{]}}
  \end{figure}
\item
