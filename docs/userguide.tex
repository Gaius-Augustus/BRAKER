...
\begin{itemize}
 ...
 \item genome file and database of proteins that may be of longer evolutionary distance to the target species (see figure \ref{braker-main}B; this approach is suitable if no RNA-Seq data is available, and if no protein data from a very closely related species is available,
 ...
 \item genome and RNA-Seq file and proteins of short evolutionary distance (see figures \ref{braker2-sidetrack}C and \ref{braker2-sidetrack}E). In both cases, GeneMark-ET is trained supported by RNA-Seq data, and the resulting gene predictions are used for training AUGUSTUS. In approach C), protein alignment information is used in the gene prediction step with AUGUSTUS, only. In approach E), protein spliced alignment data is used to complement the training set for AUGUSTUS. The latter approach is in particular suitable if RNA-Seq data does not produce a sufficiently high number of training gene structures for AUGUSTUS, and if a very closely related and already annotated species is available. \textbf{Carefully inspect results of BRAKER2 runs with RNA-Seq and protein data! Compare to runs with only one of those data sources. The optimal result is not always the one from integrating both sources of evidence. (This will change in future versions of BRAKER2.)}
\end{itemize}
...
\section{Testing BRAKER2 with RNA-Seq (only) data}

\subsection{Testing Training \& Prediction}
...
\subsection{Prediction only}

The following command will test the AUGUSTUS prediction module of braker.pl with pre-trained parameters (e.g.~the \texttt{fly} parameters from AUGUSTUS release):

\begin{verbatim}
braker.pl --genome=genome.fa --bam=RNAseq.bam --species=fly --skipAllTraining
\end{verbatim}

\section{Testing BRAKER2 with hints from proteins of remote homology (only)}

The following command will test the pipeline according to figure \ref{braker-main}B (training and prediction):

\begin{verbatim}
braker.pl --genome=genome.fa --hints=ep.hints --epmode
\end{verbatim}

\section{Testing BRAKER2 with hints from proteins of remote homology and RNA-Seq}
...
\section{Testing BRAKER2 with proteins of close homology (only)}

\subsection{Testing Training \& Prediction}

The following command will test the pipeline according to figure \ref{braker2-sidetrack}D (training and prediction):
...
\begin{verbatim}
braker.pl --genome=genome.fa --prot_seq=prot.fa --prg=gth --species=fly --skipAllTraining
\end{verbatim}

\section{Testing BRAKER2 
