...
Export ```BRAKER/scripts``` to your ```$PATH``` variable, e.g:
```
...
* ```genome.fa``` - genome sequence in FASTA format that has been softmasked
...
#### BRAKER1
Make sure that your ```$GENEMARK_PATH``` and ```$AUGUSTUS_CONFIG_PATH``` paths are set correctly
...
In this protocol, you map the transcripts from the subread libraries from long-read RNA-Seq to the genome sequence and use GeneMarkS-T to identify protein-coding regions in the transcripts.
...
```
Concatenate the subread libraries files into a single file (note that you have to replace ```subreads1.fastq,subreads2.fastq,subreads3.fastq``` with your list of files in following command):
```
...
```
You can find the collapsed transcripts at ```cupcake.collapsed.gff``` .
...
The final result is located at ```$wdir/tsebra/tsebra.gtf``` .
